...
function map(data) {
  data.forEach(function (item) {
    ...
    var icon = item.isPrivate ? 'lock' : 'comments';
    item.icon = _react2['default'].createElement(_iconIcon2['default'], { name: item.name, className: 'fa fa-' + icon + ' ' + sheet.classes.icon });
  });
