...
function URLManager() {
	...
	this.router.on(/^\/chat\/\@([^\/]+)\/[\w]{32}$/, selectPM.bind(this), goToPM.bind(this));
	this.router.on(/^\/chat\/([^@\/]+)$/, selectRoom.bind(this), goToRoom.bind(this));
	this.router.on(/^\/chat\/([^@\/]+)\/([\w]{32})$/, selectRoom.bind(this), goToRoom.bind(this));
	this.router.listen();
	...
	function selectRoom(context, next) {
		var	slug = context.path.split('/')[2].toLowerCase(),
			chosenRoom = false;
		...
		if (!chosenRoom) return this.router.go('/chat/');
		context.selectRoom = chosenRoom;
		if (next) next();
	}

	function selectPM(context, next) {
		var	username = context.path.split('/')[2].substr(1),
			chosenPM = false;
		...
		if (!chosenPM) return this.router.go('/chat/');
