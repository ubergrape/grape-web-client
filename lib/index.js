...
App.prototype.heartbeat = function App_heartbeat(app) {
	if (app.connected) {
		...
		var heartbeatTimeout = setTimeout(function() { 
				console.log("NO PONG");
				// just close the socket, the on close handler should do the rest
				if (app.ws && app.ws.readyState !== app.ws.CLOSED)
					app.ws._socket.close();
		}, HEARTBEAT_TIMEOUT);
...
App.prototype.onDisconnect = function App_ondisconnect() {
	...
	if (this.ws && this.ws.readyState !== this.ws.CLOSED) {
		// make sure the socket is really closed
		// the actual ws wrapper doesnt have a close method :(
		this.ws._socket.close();
	}
	this.emit('connection closed', this.ws);
	this.reconnect();
}
/**
...
App.prototype.connect = function App_connect(ws) {
	...
	this._ws = ws;
	if (typeof ws === 'string')
		this.ws = new WebSocketBuffering(ws);
	this.wamp = new Wamp(this.ws, {
		omitSubscribe: true
	});
	this.ws.on('close', function() {self.onDisconnect()});
