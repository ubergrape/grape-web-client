...
App.prototype.init = function App_init(options, fn) {
	...
	self.wamp.call(PREFIX + 'users/get_profile', function (err, res) {
		...
		self.loadOrganization(self.organizations[0].id, fn);
	});
};

App.prototype.loadOrganization = function App_loadOrganization(id, fn) {
	var self = this;
	...
	function join() {
		...
		self.wamp.subscribe(PREFIX + 'organization/' + id + '#status', function (status) {
			var user = models.User.get(status.user);
			user.status = status.status;
		});
		self.wamp.call(PREFIX + 'organizations/join', id, function (err) {
			if (err)
				return fn(err);
			load();
		});
	...
	function load() {
		self.wamp.call(PREFIX + 'organizations/get_organization', id, function (err, res) {
			self.users = array(res.users.map(function (u) {
				var user = models.User.get(u.id) || new models.User(u);
				user.status = u.status;
				return user;
			}));
			self.rooms = array(res.rooms.map(function (room) {
