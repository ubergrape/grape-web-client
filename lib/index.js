...
App.prototype.setRead = function App_setRead(room, line) {
	...
	this.wamp.call(PREFIX + 'channels/read', room.id, line.id, function () {
		// ignore for now
	});
};

App.prototype.setTyping = function App_setTyping(room, typing) {
	this.wamp.call(PREFIX + 'channels/set_typing', room.id, typing, function () {
		// ignore
	});
};
...
App.prototype.subscribeRoom = function App_subscribeRoom(room) {
	...
	var uri = [PREFIX + 'organization', org, 'channel', room.id].join('/');
	...
	this.wamp.subscribe(uri + '#typing', function (msg) {
		//var user = models.User.get(msg.user);
		if (msg.typing && !room.typing[msg.user]) {
			room.typing[msg.user] = true;
			trigger();
		} else if (!msg.typing && room.typing[msg.user]) {
			delete room.typing[msg.user];
			trigger();
		}
		function trigger() {
