...
App.prototype.joinRoom = function App_joinRoom(room, fn) {
	...
	this.wamp.call(PREFIX + 'rooms/join', room.id, function (err, res) {
		if (err)
			return fn(err);
		room.joined = true;
		// FIXME: make sure we add the user in case the server does not send the event
		if (!~room.users.indexOf(self.user))
			room.users.push(self.user);
		fn(err, res);
	});
};
App.prototype.leaveRoom = function App_leaveRoom(room, fn) {
	...
	this.wamp.call(PREFIX + 'rooms/leave', room.id, function (err, res) {
		if (err)
			return fn(err);
		room.joined = false;
		// FIXME: make sure we remove the user in case the server does not send the event
		var index = room.users.indexOf(self.user);
		if (~index)
