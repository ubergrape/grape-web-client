...
App.prototype.deleteRoom = function App_deleteRoom(room) {
	room.organization = this.organization.id;
	var self = this;
	this.wamp.call(PREFIX + 'rooms/delete', room.id, function (err, room) {
		if (err) return self.emit('roomdeleteerror', errorJSON(err));
