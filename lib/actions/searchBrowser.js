...
function selectSearchBrowserItem() {
  return function (dispatch, getState) {
    var _searchBrowserSelector = _selectors.searchBrowserSelector(getState());

    var data = _searchBrowserSelector.data;
    var focusedItem = _searchBrowserSelector.focusedItem;

    if (focusedItem.type === 'filters') {
      var service = _componentsBrowserDataUtils.findById(data.services, focusedItem.id);
      var filters = service ? [service.key] : [];
      dispatch({
