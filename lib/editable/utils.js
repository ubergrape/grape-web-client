...
function findGrapeObjects(node) {
  return _lodashLangToArray2['default'](_componentQuery2['default'].all('[data-object]', node));
}
...
function getText(node) {
  ...
  findGrapeObjects(newNode).forEach(function (el) {
    if (options.skipObjects) remove(el);else replace(el, document.createTextNode(el.dataset.object));
  });
