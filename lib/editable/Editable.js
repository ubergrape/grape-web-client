...
var Editable = (function (_Component) {
  ...
  function Editable(props) {
    ...
    this.onKeyDownDebounced = _lodashFunctionDebounce2['default'](this.handleKeyDown.bind(this), 20);
  }
  ...
  Editable.prototype.replaceQuery = function replaceQuery(replacement, options) {
    var query = this.getQuery();
    if (!query) return false;

    this.modify(function (left, right) {
  ...
  Editable.prototype.modify = function modify(modifier) {
    var _this = this;

    var options = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

    var selection = this.caret.placeMarker();
    var caretsParent = this.caret.getParent(selection);
    var html = utils.htmlWhitespacesToText(caretsParent.innerHTML);
    ...
    this.scribe.transactionManager.run(function () {
      ...
      _this.afterInsertionAnimation();
      _this.props.onChange(options.query);
    });
  };
  ...
  Editable.prototype.getQuery = function getQuery() {
    ...
    var text = this.caret.getText('before');
      ...
      case 'backspace':
        if (inNonEditable) {
          ...
          this.scribe.transactionManager.run(function () {
            ...
            utils.removeEmpty(parent, _this3.node);
          });
        }
        break;
      case 'esc':
  ...
  Editable.prototype.afterInsertionAnimation = function afterInsertionAnimation() {
    ...
    setTimeout(function () {
      utils.findGrapeObjects(_this4.node).forEach(function (el) {
  ...
  Editable.prototype.ensureNewLine = function ensureNewLine(key, e) {
    ...
    this.scribe.transactionManager.run(function () {
      var selection = _this5.caret.placeMarker();
      var caretsParent = _this5.caret.getParent(selection);
  ...
  Editable.prototype.onChangeDebounced = function onChangeDebounced() {
    var changeDelay = this.props.changeDelay;

    if (!changeDelay) return this.onChange();
    clearTimeout(this.changeTimeoutId);
    this.changeTimeoutId = setTimeout(this.onChange.bind(this), changeDelay);
  };
