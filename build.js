...
program
  .usage('./build [scripts] [styles] [files]')
...
var jade = require('./builder-jade-virtualdom');
...
function buildScripts(tree) {
  ...
  .end(function (err, js) {
    ...
    // add all necessary plugin scrips to output, especially require()!!!!!!!
    js = build.scripts.require + js;
    // js = jade.runtime + js;  // swatinem's jade doesn't need this!
