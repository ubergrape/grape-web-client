...
function renderAutocompleteItem(obj, asButton) {
	...
	if (asButton===true) {
		if (isWebkit()) {
			...
			return '<button class="ac ' + obj.service + '" contenteditable="false" tabindex="-1" data-object="' + obj.id + '" data-url="' + obj.url+ '">' + obj.insert + '<span class="entry-type-icon type-' + obj.service + obj.type +'">&nbsp;</span></button>';
		} else {
			// Firefox, IE
