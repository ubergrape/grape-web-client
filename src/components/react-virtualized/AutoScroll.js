...
export default class AutoScroll extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    //     console.log('rowsHasChanged PPPPPPP')
    ...
    //   console.log('rowsHasChanged OOOOOOOO')
    ...
    if (this.scrollTop === 0 && this.notZeroScrollTop) {
      console.log('SCROLL TO POSITION', this.notZeroScrollTop)
      nextProps.scrollToPosition(this.notZeroScrollTop)
    }
  }

  onScroll = ({scrollHeight, clientHeight, scrollTop}) => {
    console.log('onScroll - scrollHeight/scrollTop', scrollHeight, scrollTop)
    if (this.scrollTop !== undefined) {
      this.direction = scrollTop - this.scrollTop
    }
    if (scrollTop !== 0) {
      this.notZeroScrollTop = scrollTop
    }
