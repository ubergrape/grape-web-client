...
export default function reduce(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case types.GO_TO_CHANNEL:
      ...
      if (payload.channelId === payload.currentChannelId) return state
    ...
    case types.REQUEST_POST_MESSAGE: {
      ...
      if (payload.message.channelId !== payload.channel.id) {
        return state
      }
    ...
    case types.ADD_NEW_MESSAGE: {
      const { message: newMessage, currentUserId, currentChannelId } = payload

      if (newMessage.channelId !== currentChannelId) return state
      // Do not append a message if the history is not up to date
      if (state.backendHasNewerMessages) return state
      ...
      const scrollTo =
        newMessage.author.id === currentUserId ? newMessage.id : null
      return {
        ...state,
        scrollTo,
        scrollToAlignment: null,
